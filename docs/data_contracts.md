...
- `.uns['cost_matrix']`: Canonical cost matrix on the shared state/prototype axis, generated by the official representation route in `src/slotar`.
...
- A `cost_matrix` of shape `[K, K]` on the same shared axis.
- Required global scaling objects such as `s_C` where mandated by the method.
